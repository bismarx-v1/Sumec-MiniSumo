...
void postAll() {	//"all" post request response
	...
	if(error) {
		...
		const char* VarMode = doc_server_rx["Mode"];
		const char* VarTfL_Left = doc_server_rx["TfL_Left"];
		const char* VarTfL_Mid = doc_server_rx["TfL_Mid"];
		const char* VarTfL_Right = doc_server_rx["TfL_Right"];
		const char* VarSharp_Left = doc_server_rx["Sharp_Left"];
		const char* VarSharp_Right = doc_server_rx["Sharp_Right"];
		const char* VarQRE_Left = doc_server_rx["QRE_Left"];
		const char* VarQRE_Rear = doc_server_rx["QRE_Rear"];
		const char* VarQRE_Right = doc_server_rx["QRE_Right"];
		const char* VarIR = doc_server_rx["IR"];
