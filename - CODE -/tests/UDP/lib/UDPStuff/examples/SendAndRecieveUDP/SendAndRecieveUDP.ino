...
void loop() {
	...
	CheckIfRecieved(&ReturnSize, ReturnMessage, &ReturnIp);	// use this to check for new messages
	if(ReturnSize > 0) {
	...
	if(millis()-TimePeriodSend >= TimeLastSend) {
		TimeLastSend = millis();
		SendUdpToAll("Test1", 5, UdpPort);	// sends string "Test1" 5 times to all connected devices 
	}
